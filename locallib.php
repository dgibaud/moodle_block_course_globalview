...
/**
 * Helper functions for course_globalview block
 *
...
/**
 * Display globalview for courses
 *
 * @param array $courses courses for which globalview needs to be shown
 * @return array html globalview
 */
function block_course_globalview_get_globalviews($courses) {
    $htmlarray = array();
    if ($modules = get_plugin_list_with_function('mod', 'print_globalview')) {
...
/**
 * Sets user preference for maximum courses to be displayed in course_globalview block
 *
 * @param int $number maximum courses which should be visible
 */
function block_course_globalview_update_mynumber($number) {
    set_user_preference('course_globalview_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in course_globalview block
 *
 * @param array $sortorder sort order of course
 */
function block_course_globalview_update_myorder($sortorder) {
    set_user_preference('course_globalview_course_order', serialize($sortorder));
}
 ...
 */
function block_course_globalview_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_course_globalview', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_course_globalview', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_course_globalview', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in course_globalview block
 *
 * @param bool $showallcourses if set true all courses will be visible.
 * @return int maximum number of courses
 */
function block_course_globalview_get_max_user_courses($showallcourses = false) {
    // Get block configuration
    $config = get_config('block_course_globalview');
    $limit = 0; //$config->defaultmaxcourses;
    ...
    if (empty($config->forcedefaultmaxcourses)) {
        if ($showallcourses) {
            ...
            $limit = get_user_preferences('course_globalview_number_of_courses', $limit);
        }
    }
    return $limit;
}
 ...
 */
function block_course_globalview_get_sorted_courses($showallcourses = false) {
    global $USER;

    $limit = block_course_globalview_get_max_user_courses($showallcourses);
    ...
    $order = array();
    if (!is_null($usersortorder = get_user_preferences('course_globalview_course_order'))) {
    ...
    // From list extract site courses for globalview
    $sitecourses = array();
